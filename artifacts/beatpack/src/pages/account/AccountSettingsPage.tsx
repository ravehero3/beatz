  ...
  function handleSave(e: React.FormEvent) {
    ...
    updateMyProfile.mutate(
      { data: { firstName: firstName || null, lastName: lastName || null } },
      {
        ...
        onError: () => setMsg({ text: "Failed to update profile. Please try again.", ok: false }),
      }
    );
  }

  const roleLabels: Record<string, string> = { buyer: "Buyer", artist: "Artist", admin: "Admin" };

  return (
    <div style={{ paddingTop: "44px", minHeight: "100vh", background: "#F9F9F9" }}>
      <div style={{ maxWidth: "560px", margin: "0 auto", padding: "40px 24px" }}>
        <div style={{ marginBottom: "32px" }}>
          <h1 style={{ fontFamily: F, fontWeight: 700, fontSize: "24px", color: "#0A0A0A", letterSpacing: "-0.02em", marginBottom: "4px" }}>Account Settings</h1>
          <p style={{ fontFamily: F, fontSize: "14px", color: "#888888" }}>Manage your profile information.</p>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
          ...
          <div style={{ background: "#FFFFFF", border: "1px solid #E5E5E5", borderRadius: "16px", padding: "24px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "20px" }}>
              ...
              <span style={{ fontFamily: F, fontWeight: 600, fontSize: "16px", color: "#0A0A0A" }}>Profile</span>
            </div>
            ...
            <form onSubmit={handleSave} style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
                <Field label="First name">
                  <Input value={firstName} onChange={setFirstName} placeholder="Jana" />
                </Field>
                <Field label="Last name">
                  <Input value={lastName} onChange={setLastName} placeholder="Nováková" />
                </Field>
              </div>
          ...
          <div style={{ background: "#FFFFFF", border: "1px solid #E5E5E5", borderRadius: "16px", padding: "20px" }}>
            <div style={{ fontFamily: F, fontWeight: 600, fontSize: "14px", color: "#0A0A0A", marginBottom: "12px" }}>Account Info</div>
            <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
              {[
                { label: "Role", value: roleLabels[user?.role ?? ""] ?? user?.role ?? "—" },
                { label: "Member since", value: user?.createdAt ? new Date(user.createdAt).toLocaleDateString("cs-CZ", { day: "numeric", month: "long", year: "numeric" }) : "—" },
              ].map(({ label, value }) => (
          ...
          {/* Password reset hint */}
          <div style={{ background: "#F9F9F9", border: "1px solid #E5E5E5", borderRadius: "16px", padding: "16px 20px" }}>
            <div style={{ fontFamily: F, fontSize: "13px", color: "#888888", lineHeight: 1.6 }}>
              To change your password, use the{" "}
              <a href="/forgot-password" style={{ color: "#0A0A0A", fontWeight: 600, textDecoration: "underline" }}>forgot password</a>
              {" "}flow — we'll send you a reset link by email.
            </div>
          </div>
        </div>
      </div>
